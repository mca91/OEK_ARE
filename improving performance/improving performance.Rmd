...
.blockquote.exercise[
#### `r icon::fa("desktop")` Example: coercion of inputs / robustness checks
...
## 2. Do as Little as Possible
.smaller[
A function is faster if it has less to do &mdash; obvious but often neglected! Use functions tailored to specific input and output types / specific problems.
.blockquote.exercise[
#### `r icon::fa("desktop")` Example: row sums &mdash; `rowSums()` vs. `apply()`
...
Note that `any()` calls the primitive `any`.

---
## 2. Do as Little as Possible

.blockquote.exercise[
#### `r icon::fa("desktop")` Example: searching a vector
...
.smaller[
.blockquote.exercise[
#### `r icon::fa("desktop")` Example: Linear Regression &mdash; computation of $SE(\widehat\beta)$
...
.smaller[
.blockquote.exercise[
#### `r icon::fa("desktop")` Example: Linear Regression &mdash; computation of $SE(\widehat\beta)$ &mdash; ctd.
...
R's interpreter performs a number of costly operations. The `compiler` package reduces the resulting slowdown by providing a translator which generates a byte compiled version of an R function.
...
`bytecode: <...>` points to the memory location of the *compiled* function

---
## 2. Do as Little as Possible &mdash; Byte Compiler
.smaller[
.blockquote.exercise[
#### `r icon::fa("desktop")` Example: compiled correlation matrix
Interpreting the code is more demanding than the actual 'computation'.
...
.smaller[
.blockquote.exercise[
#### `r icon::fa("desktop")` Example: compiled correlation matrix &mdash; ctd.
...
- `:` is a primitive and is faster than `seq()`. We circument generating the sequence and use `sample.int(n, n)`.
- Passing vectors using `$` is faster than looking up the correct method of $[$
- `cor()` runs faster on vectors
...
1[1]
```
Scalar operations thus work on vector *elements* which (often) is needlessly cumbersome.
]
...
- Loops are often used in an *inefficient* manner: 
    ...
    growing an object with improper initialisation means overhead due to **growing and copying** in every iteration.

**What if I need to use `for()`?**

Allocate required storage before the loop and *fill* the allocated object!

(BTW, this is what `apply()` does under the hood.)
...
Technically this does not directly relate to vectorisation but it demonstrates that growing objects using loops is a bad idea. A vectorised approach is often faster.
...
1. Which approach is faster `if()` or `ifelse()`?

2. Compare the speed of `apply(X, 1, sum)` with the vectorised `rowSums(X)` for varying sizes of the square matrix `X` using `bench::mark()`. Consider the dimensions `1`, `1e1`, `1e2`, `1e3`, `0.5e4` and `1e5`. Visualize the results using a violin plot.
...
4.  Find an approach to compute column maxima of a numeric matrix `X` using a `*apply()`-function which outperforms `apply(X, 2, max)`.
    ```{r, include=F}
    X <- matrix(rnorm(1000), 
                ncol = 100)
...
1. `ifelse()` is vectorized, `if()` is not. However `ifelse()` evaluates both the condition and the `else` statement whereas `if()` is lazy. `if()` is only faster when the condition is of length one.
...
.smaller[
.blockquote.exercise[
#### `r icon::fa("desktop")` Example: send them cores to sleep
```{r, eval=F}
...
.smaller[
.blockquote.exercise[
#### `r icon::fa("desktop")` Example: send them cores to sleep &mdash; ctd.
What happened?
...
- Running MC simulations is an everyday task in econometrics. They are routinely used for approximating quantities which cannot be derived analytically, e.g., finite sample properties of a test statistic.
...
The `MonteCarlo` package streamlines the process of writing MC simulations. Useful features are:
