...
The first step is to load the website into R using `xml2::read_html()`. Since `rvest` depends on `xml2`, the `xml2` package is automatically loaded when `rvest` is loaded.
...
```{r, eval = T}
library(rvest)
URL <- here::here("SoSe_2025/Webscraping/examples/simple_html_page.html")
(page <- read_html(URL))
```
]
...
#### `r desktop` Example
```{r, eval = T}
url <- here::here("SoSe_2025/Webscraping/examples/simple_html_page_with_css.html")
page <- url %>% read_html()
page %>%  
  html_nodes(".red") %>% 
  html_nodes("p") %>%
