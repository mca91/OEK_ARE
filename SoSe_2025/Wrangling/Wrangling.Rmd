...
summarise(mtcars_tibble, my_variance(mpg))
```
...
summarise(mtcars_tibble, two_return_vaules(mpg))
```
]
...
- Inside the `across()` function we can use the helper functions to select certain variables which we already saw on the previous slides. 
