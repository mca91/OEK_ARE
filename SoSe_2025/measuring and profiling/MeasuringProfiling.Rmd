...
- **Profiling** is measuring run time *line-by-line* using realistic inputs to **identify bottlenecks**.
...
- `utils::Rprof()` is **sampling profiler**: it tracks the **function call stack** at regularly sampled intervals.

- Results are *stochastic*&mdash;we never run a function under the same conditions twice 
    
    (think memory usage, CPU load, etc.) 
...
.smaller[
.content-box-white[
**Task:** profiling nested functions
...
.smaller[
.blockquote.exercise[
#### `r desktop` Example: profiling linear regression and visualization
...
.blockquote.exercise[
#### `r desktop` Exercise: copy-on-modify and extensive garbage collection
...
---
## Profiling &mdash; Notes and Hints

<br>

- We **cannot profile compiled code** in R (`C`/`C++`/`FORTRAN`)

    $\Rightarrow$ Cannot measure the call stack of **primitives** (`sum()`, `sqrt()`, ...) 

    $\Rightarrow$ Cannot use profiling to see whether the code is slow because something further down the call stack is slow.
  
- Profiling is another good reason to **break your code into functions** (Functional programming!) so that the profiler can give useful information about where time is being spent
...
- **Important**: 
    ...
    Microbenchmarks can rarely be generalised to 'real' code: Observed timings in benchmarks are typically dominated by **higher-order effects** in real code.
  
  **A deep understanding of quantum physics is not very helpful when baking cookies.**
  
- There are several R packages for microbenchmarking. We will rely on the `bench` package by Hester (2025) which has the most accurate timer function currently available in `R`.
...
- `bench` uses the **highest precision APIs** available for the operating system (often nano-seconds!)

- `mark()` is the working horse of the `bench` package and measures both memory allocation and computation time.

- By default, a **human-readable statistical summary** on the distributions of **memory load** and **timings based on multiple iterations** is returned which also reports on garbage collections

- Benchmarking across a **grid of input values** with `bench::press()`

- The package also has methods for **neat visualisations** of the results using `ggplot2::autoplot()` (default plot type is [beeswarm](https://flowingdata.com/2016/09/08/beeswarm-plot-in-r-to-show-distributions/)) 
...
Sometimes we want to benchmark against a parameter grid. 

$\Rightarrow$ `bench::press()` to the rescue!
...
- `r emoji::emoji('bolt')` tweaking for speed improvements is "easier" (and done by R-core team) but not feasible for end users like us. <br><br> Some alternatives to the "official R" address specific issues:

  - "pretty quick R" [pqR](http://www.pqr-project.org/) (parallelisation of core functions)
  
  - [Microsoft R Open](https://mran.microsoft.com/open) (discontinued, some fast extensions for machine learning)
...
---
## (Why) is R slow? &mdash; Performance Drop from Parsing
...
---
## (Why) is R slow? &mdash; Promise Creation Isn’t Free
...
- R isn’t slow *per se* — but it **is slower than many other languages**.

  R was designed to make your life easier, not your computer’s.

  It's a high-level language built for data analysis and statistics — far easier than doing it all in C!
  
  As of `r Sys.Date()`, there are `r nrow(available.packages(repos = "http://cran.us.r-project.org"))` packages on [CRAN](https://cran.r-project.org/) — most problems have already been solved for you.

- We're (mostly) happy to accept the trade-off: **slower execution for much faster development**.

- We can't fix R's quirks, but:  
  
  **You *can* write efficient code** — if you know what to watch out for.

  Stay tuned — we’ll look at more practical tips in the next lectures.
