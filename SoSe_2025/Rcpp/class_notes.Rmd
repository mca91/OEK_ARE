---
title: "Rcpp --- Solutions to Exercises on Lecture Slides"
output: pdf_document
---
...
- Find out why the following code gives a compile error:
    ...
    **Solution:**
    ...
    The namespace should be referenced after including the headerfile `Rcpp.h`: `using namespace Rcpp;`
    
    `x[id]` returns a subview class of `Rcpp::Vector` which is *not* a double. It is important to note that coercion to another type is not as easily done as in `R`! We need to get the integer entry from the numeric vector `id` for subsetting: `x[id[0]]`
