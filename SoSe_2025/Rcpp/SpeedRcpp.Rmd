...
- We **need to declare the return *type*** (also for function arguments)

- The **<tt>return</tt> statement** *must* be used to return a value!

- We **conclude statement with <tt>;</tt>**

- **Scalars and vectors are different objects**! There are also differences w.r.t. scalar types: 

    `numeric`, `integer`, `character` and `logical` (in `R`)
    
    `double`, `int`, `string`, `bool` (in `C++`)
...
The following code is pure `C++` and can be handled by `Rcpp`: `std::vector<type>` is mapped to `NumericVector`. 

(We use `::` to access objects from the `std` namespace)
