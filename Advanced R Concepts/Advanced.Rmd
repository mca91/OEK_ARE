...
- For consistency we (have and will) use `<-` for binding and `=` for assigning function arguments.
...
- If `<<-` does not find a corresponding binding in the enclosing environment, it looks in the parent environment and works it's way up towards to global environment. If the binding does not exist there it will be created.
...
R's copy-on-modify behavior is blessing and curse: we generally may use references to an object without the risk of breaking existing code.
...
- Question: object addresses (like `0x7f9ef3059d38`) will be different if the code is re-run. Why?
...
- **`f(x)`**: `f()` binds `a` to the same memory location `x` points to *during execution*. A copy is made since `f()` modifies `a`.
...
Which code statement generates the result shown in each diagram? What is special about the last one?
...
- `x` is referenced more than once: 
    ...
    &rarr; modifying `x` results in a copy
...
**Scoping** refers to the routine of finding the value associated with a name. R's scoping mechanism follows four concepts you should already be familiar with.
...
It is straightforward to write your own operators in infix form.
...
Replacement functions have the below form.
...
- Note that replacement functions must have `x` and `value` as arguments and must return the modified object `x`
