...
R's copy-on-modify behavior is blessing and curse: 

- We may use references without the risk of breaking existing code.

- _modifying_ a reference may trigger a copy of the value.
...
Which statement generates the result shown in each diagram? What is special about the bottom one?
...
A _data frame_ is a list object whose elements point to column vectors. 
...
- The GC runs automatically if space is needed for creating new objects. There is *no need to actively force garbage collection*. You can, however, do so by calling `gc()` with the side effect of info on memory occupation (there's also a button in RStudio for this).
...
R functions are **objects**! We may do stuff that may seem exotic when compared to  languages like _C_ and _Python_ that demand very explicit definitions.
...
Replacement functions are very convenient but there is no free lunch: replacements *always* trigger a copy.
