...
.pull-right-2[
<p> <span style="margin-bottom: -7px;">Martin Schmelzer </span><span><a href = "mailto: martin.schmelzer@outlook.com">Martin.Schmelzer@uni-due.de</a></span></p> 
...
.center[
  https://lehre.moodle.uni-due.de/course/view.php?id=12923  
]
<br>
...
- Your have to register for the examination at your examination office as usual.

- You will have to work on a final group project which focuses on one or more of the topics discussed in the course. 

- We will provide a few project ideas but welcome you to come up with your own.

- Projects should be registered with us until June 26th. We will set up a form on Moodle.

- Oral presentations of the projects will take place October 8th.

- The final grade is a weighted average of the grades for the group project (50%) and its presentation (50%).
...
7. Git & Github <!--(Git/git_vcs.html)-->
]

.pull-right[
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**Lecturer: Martin Arnold**
<ol start = "8">
  <li><a href="Advanced R concepts/Advanced.html" class="disabled">Advanced Concepts</a></li>
...
- **Advantages:**
  ...
  - R has the capabilities to connect to high-performance programming languages like C, Fortran, and C++, ...
...
#### The main sources for packages are:
  - CRAN (Comprehensive R Archive Network) with as of now ca. `r format(nrow(available.packages(repos = "https://cran.r-project.org")), big.mark = ".")` available packages
...
- E.g., use `remotes::install_github("tidymodels/broom")` to install the `broom` package from Github.
...
???
Similar function for bioconductor
...
But stackoverflow is an endangered species:
https://blog.pragmaticengineer.com/content/images/size/w1000/2025/05/image-6.png
...
1. A positive integer selects the value at that position. A negative integer *de*selects the value at that position. With a logical subsetting vector we can filter based on their position using TRUE (select) and FALSE (unselect). A character vector leads to errors ...

2.[] = always returns object of same class -> we get a list, [[]] = can extract one element from a list or data frame
...
???
ifelse () is a vectorized function that is applied on each value of a vector while
if() expects one logical value and is used to control the flow of a script
...
- R uses lexical scoping:  R looks up the values of names where the function was defined, not where it is called from.
]
...
#### `r ICONS$desktop` Example: lexical scoping
...
???
Search Path: Local Environment -> Enclosing Environment -> Parent Environments -> Empty Environment
---
## Scoping Rules
...
#### `r ICONS$desktop` Example: What happens here?
```{r, eval = FALSE}
x <- 3
...
#### `r ICONS$desktop` Example: And here?
...
#### `r ICONS$desktop` Example: Let's make things more confusing. Can you predict the result?   
...
???
Even though the variable square_2 is defined within the scope of the function square_3(), R ignores it because it as found a function named square_2 in the enclosing (global) environment)
...
#### `r ICONS$desktop` Example: What is the result of the first call to `square_4()`? What will happen the second time?
...
#### `r ICONS$desktop` Example: Predict the result of the two calls to `f1()`. Compare it to the main rule of lexical scoping. 
...
???
Lexical Scoping: R looks up the values of names based on how a function is defined, not how it is called
...
#### `r ICONS$desktop` What could cause an error here?
```{r}
...
#### `r ICONS$desktop` Function arguments are lazily evaluated; they’re only evaluated if accessed.
...
```{r}
df_list <- lapply(0:4, function(j) {
  data.frame(c1 = (1:5) + 5 * j, c2 = letters[(1:5) + 5 * j])
})
...
```{r}
# 1.
do.call(what = rbind, args = df_list)
rbind(df_list[[1]], df_list[[2]], df_list[[3]], df_list[[4]], df_list[[5]])

# 2.
mtcars[mtcars$cyl == 4, ]
mtcars[-c(1:4), ]
mtcars[mtcars$cyl <= 5, ]
mtcars[mtcars$cyl == 4 | mtcars$cyl == 6, ]  # Or: mtcars[mtcars$cyl %in% c(4, 6), ]
