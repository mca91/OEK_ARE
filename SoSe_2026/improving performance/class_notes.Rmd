...
## Slide 10: 2. Do as Little as Possible

**Example: method dispatch and primitives**

```{r}
x <- runif(1e2)

bench::mark(
  mean(x),            # generic: dispatches to mean.default()
  mean.default(x),    # explicit: skips dispatch
  sum(x)/length(x)    # primitive: fastest
)
```

- `mean()` is a generic function that must first locate the right method before executing --- this lookup is called method dispatch and has a cost.

- Being explicit (`mean.default()`) skips dispatch and is faster.

- `sum(x)/length(x)` uses only primitive functions which bypass the R interpreter entirely, making it the fastest option.
...
1. Can you come up with an even faster implementation of `b()` in the linear regression example?
    ...
    - Exploiting that `X` is the only regressor allows us to replace the regressor matrix product and use a more specific approach to compute $SE(\widehat\beta_1)$ in `d()`
    ...
    A disadvantage is that the faster approaches `b()` and `d()` are inflexible and error prone: an inexperienced user is likely to supply inputs which will cause the function to crash or return false results--note that we don't do any checks of the input!
...
## Slide 26: Vectorise your Code --- Exercises
...
1. Compare the speed of `apply(X, 1, sum)` with the vectorised `rowSums(X)` for varying sizes of the square matrix `X` using `bench::mark()`. Consider the dimensions `1`, `1e1`, `1e2`, `1e3`, `0.5e4` and `1e5`. Visualize the results using a violin plot.
    ...
    Note that `apply()` which is not 'vectorised for performance' cannot keep up with `rowSums()`: it is clearly outperformed by the _C_ internals, especially if dimensions are large. This is because the `rowSums()` and its internal _C_ functions need less (costly) function calls and is more memory efficient. 
...
2. 
    ...
    (b) `crossprod()` computes the dot product which is also a weighted sum:
        ...
        We subset because the return value is a matrix, see `?crossprod`. We can verify this equals `sum(a * w)`:
        
        ```{r}
        sum(a * w) - crossprod(a, w)[1]   # should be approx. 0
        ```
...
The `furrr` package provides parallel equivalents of all `purrr` functions using the `future` backend --- a cross-platform drop-in replacement. Switching from sequential to parallel execution requires minimal code changes: replace `map()` with `future_map()`, `map_dbl()` with `future_map_dbl()`, etc.

```{r, eval = F}
library(furrr)

# Set up a parallel plan using 4 sessions
plan(multisession, workers = 4)
```

The following example runs a bootstrap to estimate the sampling distribution of the slope coefficient from regressing `mpg` on `wt` in the `mtcars` dataset.

```{r, eval = FALSE}
boot_coef <- function(i) {
  idx <- sample(nrow(mtcars), replace = TRUE)
  coef(lm(mpg ~ wt, data = mtcars[idx, ]))["wt"]
}

system.time(map(1:2000, boot_coef))           # sequential
system.time(future_map(1:2000, boot_coef))    # parallel

plan(sequential)  # always reset when done
```

- `future_map()` pays off when each iteration is **expensive and independent**. For cheap tasks, overhead dominates --- just as with `mclapply()`.

- Works on Windows, Mac, and Linux (unlike `mclapply()`) and integrates naturally into `purrr`-style pipelines.

- Always call `plan(sequential)` when done to release the parallel workers.
