...
- You’ll also need to install the `DBI` (backend) package. The `DBI` package provides a common interface that allows `dplyr` to work with many different databases using the same code. 
...
#### `r ICONS$desktop` Setup the connection 
...
#### `r ICONS$desktop` Example 
```{r, eval = FALSE}
DBI::dbWriteTable(con, name = "tbl_gapminder", value = gapminder::gapminder)
```
]
...
```{r, eval = FALSE}
new_row <- data.frame(country = "Gondor", continent = "Middleearth", year = 251, 
                      lifeExp = 72.1, pop = 435961, gdpPercap = 12.34)
DBI::dbWriteTable(con, name = "tbl_gapminder", value = new_row, append = TRUE)
...
--
.font80[
- Always make sure, your database connection is being closed using `dbDisconnect`
]


???
Closeing connections: pool manages connections for you 
...
1. Set up a new SQLite database containing the Boston Crime data (in the same folder where your RMarkdown report is located).

2. Rewrite your RMarkdown report on crime so that
  - It establishes a connection to the crime database.
  - All the other lines of code work with the new data source.
  - Make sure the database connection is closed at the end of your document.
