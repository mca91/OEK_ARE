---
title: "Monthly Report on Crime"
author: "Martin"
output: 
  gelsentemplates::handout:
    folio: yes
    frontpage:
      cover: "1"
    margin_content:
      top_left: "''"
      top_right: "''"
      bottom_right: "'R3/1 Statistik & Monitoring'"
      bottom_left: "'Seite ' counter(page)"
editor_options: 
...
summary_tbl <- tbl_df %>% tbl_summary(
    by = TimeOfDay,
    statistic = list(
      all_categorical() ~ "{p}%"
    ),
...
gt_obj <- summary_tbl %>%
  as_gt() %>%
  gt::tab_caption("Burglaries by district and time of day.") %>%
  gelsentemplates::tab_options_ge() 
