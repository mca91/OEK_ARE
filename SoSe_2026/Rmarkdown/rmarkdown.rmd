...
1. Create an R Markdown document that is compiled to an HTML document. 
   The document should consist of two main (first level) sections:
    - Crime Incidents Report Data
    - Explorative Data Analysis.
2. Use the first section to read in the "Crime Incident Reports" (`readRDS()`) and the "Field Explanation" (`readxl::read_xlsx`) files available on Moodle. Then
  - Display a sample of the data as a formatted table
  - Write 1-2 sentences that include derived information (number of observation, number of variables, ...) from the actual data set (use inline R code).
  - Look at the "Field Explanation" data and tidy it (`dplyr` + `tidyr`) 
  - Display the tidied explanation data as a formatted table
