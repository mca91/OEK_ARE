...
.font80[
R Markdown provides a framework for combining code, results, and commentary. You can use R Markdown
...
- Alternatively, hit the "Knit" button in Rstudio.
...
.blockquote.exercise[
#### `r ICONS$desktop` Example: Basic YAML header using mostly defaults
...
.blockquote.exercise[
#### `r ICONS$desktop` Example:  A more complicated YAML header
...
#### `r ICONS$desktop` Example: Headers
```markdown
     #   Header 1     
     ##  Header 2   
     ### Header 3 
...
#### `r ICONS$desktop` Inline formatting
...
#### `r ICONS$desktop`  Unordered Lists
...
#### `r ICONS$desktop` Example: chunk setup 
...
???
The stub acts like the "y-axis" of the table
...
- `gt()` creates a gt table object and uses plenty of arguments
- `tab_header()` is used to edit the header
- `tab_stubhead()` to change the stub head label
- `cols_label()` to edit column labels
- `tab_spanner()` to insert column spanners 
- `tab_footnote()` to add footnotes
- `tab_source_note()` to a source notes
- `fmt_*()` offer multiple options to format columns
- `tab_options()` to modify table options

---
For details: `?` is your friend
...
Create a new Rmarkdown document with HTML output. It should contain a section "Data Exploration" and two subsections "Data" and "Visualizations".
<ol>
<li>Create a table that displays the mtcars dataset. The table should look like a publishable report rather than a raw console output. Requirements:</li>
<ul>
<li>Add a header: Include a title ("Motor Trend Car Road Tests") and a subtitle ("Data from the 1974 Motor Trend US magazine").</li>

<li>Create a stub: Move the car names (currently row names, see <code>tibble::rownames_to_column</code>) into a formal "Stub" column (see arguments to <code>gt()</code>).</li>

<li>Format numbers: Round the <code>disp</code> column to one decimal place and the <code>wt</code> column to three decimal places.</li>

<li>Column Labels: Change the column name <code>mpg</code> to "Miles Per Gallon".</li>
<li>Install the package <code>gtExtras</code> and apply the <i>FiveThirtyEight</i> theme to your table.</li>
</ul>
</ol>
...
<ol start="2">
<li>Create a <code>ggplot</code> based on the <code>mtcars</code> data.
  <ul>
    <li>Use any aesthetic mapping and geometry you like as long as you use either the <code>colour</code> or the <code>fill</code> aesthetic.</li>
    <li>Install <code>ggthemes</code> and apply the <i>FiveThirtyEight</i> scale to your <code>colour</code>/<code>fill</code> aesthetic (look for the corresponding <code>scale_*</code> function). Also, apply the overall <i>FiveThirtyEight</i> theme.</li>
</ul>
</li>
</ol>
...
mtcars$am <- as.factor(mtcars$am)
# Interactive Plot
plot_ly(mtcars, x = ~wt, y = ~hp, z = ~qsec, color = ~am, colors = c('#BF382A', '#0C4B8E'), width = 600, height = 500) %>%
  add_markers() %>%
  layout(scene = list(xaxis = list(title = 'Weight'),
                     yaxis = list(title = 'Gross horsepower'),
                     zaxis = list(title = '1/4 mile time')))
...
````markdown
`r ''````{r}
DT::datatable( head(iris, 10), fillContainer = TRUE,  options = list(pageLength = 5))
```
````
...
<ol start="2">
<li>Create an R Markdown document that is compiled to an HTML document. The document should consist of two main (first level) sections:
    <ul>
      <li>Crime Incidents Report Data</li>
      <li>Explorative Data Analysis.</li>
    </ul>
    </li>
<li> Use the first section to read in the "Crime Incident Reports" (<code>readRDS()</code>) and the "Field Explanation" (<code>readxl::read_xlsx</code>) files available on Moodle. Then
<ul>
  <li>Display a sample of the data as a formatted table</li>
  <li>Write 1-2 sentences that include derived information (number of observation, number of variables, ...) from the actual data set (use inline R code).</li>
  <li>Look at the "Field Explanation" data and tidy it (<code>dplyr</code> + <code>tidyr</code>) </li>
  <li>Display the tidied explanation data as a formatted table</li>
</ul>
<li>Optimize your code chunks so that the document doesn't load the data every time you compile. </li>
</ol>
...
## Exercises
<ol start="5">
<li>Use the second section for an EDA. The first subsection should analyze the incidents by day of the week.
  <ul>
    <li>Use <code>dplyr</code> to get the number of incidents per month.</li>
    <li>Create a bar chart using <code>ggplot2::geom_col</code> (check the docs for infos on the aesthetics) to show the results.</li>
    <li>Create a table using <code>gt()</code> showing the numbers. Give it a caption.</li>
  </ul>
</li>
<li>
  The second subsection analyzes incidents over time.
  <ul>
    <li>Compute the number of incidents by <code>YEAR</code> and <code>MONTH</code>. Use <code>lubridate::make_date()</code> to create a new variable called <code>Date</code> (the values should have the form <code>yyyy-mm-01</code>)</li>
  <li>Create a ggplot using geometries of your choise shoing the development of the numbers over time.</li>
  <li>Make the plot interactive.</li>
</ul>
</li>
<li>Hide all your code chunks by setting a global option. </li>
...
<li>Create a PDF document containing what you did (make sure the package <code>tinytex</code> is installed).</li>
