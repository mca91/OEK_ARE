...
```{r, cache=T, fig.align='center', echo=F, out.width='500px'}
...
ggplot(d, aes(y = PageRank, x = factor(rowname, levels = d$rowname[order(d$PageRank)]))) + 
    ...
    xlab("Package") +
    ylab("Page Rank") + 
    ggtitle("Popular Packages on CRAN as of January 24, 2020") +
...
We need `Rcpp` from CRAN as well as a working C++ compiler. Regarding the latter, installation is OS dependent.
...
- At the C level, R objects are stored in the **SEXP** (S expression) data type. An SEXP essentially is a pointer.  

**So how does it work?**

- We use C++ functions which *generate* and *modify* SEXP objects faster than R is able too. 
...
- `Rcpp` generates a wrapper function in R. It then compiles and links the C++ function to the wrapper. The function now is available in R under its C++ name.
...
It sometimes will be convenient to generate objects using the STL and use wrap before returning them.
...
`Rcpp` as classes `List` and `DataFrame` which are most interesting for input to and output of functions.
...
In C++ a *pass by value* happens by default. The functions below use return type `void`, i.e., they don't return anything.
...
Note that it's also admissible to use the address-of operator `&` in variable assignment.
...
The behavior is different in `Rcpp`.
...
The function altered the values of `A` and `B` as expected. But what happened to `x`?
...
Using `clone()` we may bypass the reference on assignment and work on `B`.
...
Most functions for mathematical operations, scalar and vector summaries and search algorithms have performant sugar equivalents.
...
Since the area of the unit circle is $\pi$ we use that $$\frac{1}{N}\sum_{i=1}^N \mathcal{I}_{\left\{\sqrt{x_i^2+y_i^2}\leq1\right\}} \xrightarrow{p} \pi/4$$ for $x_i, y_i \overset{i.i.d.}{\sim} \mathcal{U}[0,1].$
...
**Contours of $x^2 \exp(-xy^2 - y^2 + 2y - 4x)$**
