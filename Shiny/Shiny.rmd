...
- The reactive source is usually the user input which are accessible through `input$some_input` It can only have dependents (it is always a parent node). 
- The reactive endpoint is usually produced by one of the render functions and assigned to `output$some_output`. It can only be dependent (it is always a child node). 
...
Whenever  `input$some_input` changes the reactive endpoints using this input are notified that they need to re-execute. 

Which reactive structure has the following code?
...
Using a reactive conductor created by `reactive()` the function `fib()` executes only once.  
...
We can solve this by making the render function producing the output unresponsive to the parameter inputs using `isolate()`. When we are ready e.g. a button can be used to start the 
computation. 
...
1. Create an app that allows `.csv` file input.
2. Display the data to the user of you App as a `DT::DataTable`. 
3. Figure out how to create an `varSelectInput` Element 
