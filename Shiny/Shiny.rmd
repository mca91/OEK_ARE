...
- The reactive source is usually the user input which are accessible through `input$inputId`. It can only have dependents (it is always a parent node). 
...
<li> Add a checkbox "only Continents" to your app. If this checkbox is activated, a new data set
  should be computed which contains for each variable the average of all countries belonging
  to one continent (for simplicity you only have to compute a simple average). 
...
## Exercises 
<ol start=6>
<li> Add to your Gapminder app a DataTable displaying the selcted data. Use a <a href = "https://shiny.rstudio.com/articles/tabsets.html">Tab Panel </a>  to allow switching between the
